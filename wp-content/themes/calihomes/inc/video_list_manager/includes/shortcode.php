...
function get_youtube_video_ID($link) {
  ...
  if (preg_match("#([\/|\?|&]vi?[\/|=]|youtu\.be\/|embed\/)([a-zA-Z0-9_-]+)#", $link, $matches)) {
    return end($matches);
  }
...
function tntGetYoutubeEmbedLink($link)
{
	...
	$embedCode = get_youtube_video_ID($link);
	if ($embedCode == false) {
		$embedCode = '';
	}
...
function tntSCVideoList($attr){
	...
	if($items > 0)
	{
	    ...
	    echo "Trang nội dung không tồn tại danh sách video."; exit();
	}
		...
		foreach ($video as $vid)
		{
			...
			switch ($vid->video_link_type)
			{
				case "1":
					...
					$vShow['videoFrame'] = '<iframe width="'.$videoWidth.'" height="'.$videoHeight.'" src="'.$linkEmbed.'" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
					break;
				case "2":
